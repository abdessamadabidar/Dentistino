...
export default function ShadeSelection({form}: shadeSelectionProps) {
            ...
            <DropdownMenuGroup>
                ...
                <div className="grid grid-cols-5 gap-x-1.5">
                        {shades.A.map((item, index) => (
                            <ToggleGroupItem className="border" key={index} value={item} aria-label="Toggle bold">
                                {item}
                            </ToggleGroupItem>
                        ))}
                    </div>
